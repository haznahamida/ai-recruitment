...
const EducationSection: React.FC<{ educations: Education[] }> = ({ educations }) => (
        ...
        <div className="space-y-4">
            {educations.length > 0 ? educations.map(edu => (
                <div key={edu.id}>
                    ...
                    <p className="text-xs text-gray-500 mt-1">{edu.startDate.split('-')[0]} - {edu.endDate.split('-')[0]}</p>
                </div>
...
const SalaryExpectationSection: React.FC<{ salary: {min: number, max: number} }> = ({ salary }) => (
...
const DocumentCard: React.FC<{ doc: DocType }> = ({ doc }) => (
...
const ApplicationHistorySection: React.FC<{ applications: ApplicationHistory[] }> = ({ applications }) => {
    const getStatusClass = (status: ApplicationHistory['status']) => {
        switch (status) {
            case 'Diterima':
            case 'Interview':
...
const CandidateDetailPage: React.FC = () => {
    ...
    const { getCandidateById } = useHrdStore();
        ...
        <div>
            ...
            <div id="profile_info" className="bg-white shadow-sm rounded-xl p-6 mb-6">
                <div className="flex flex-col sm:flex-row items-start gap-6">
                    <img src={candidate.user.avatarUrl} alt={candidate.user.name} className="w-24 h-24 rounded-full object-cover border-4 border-gray-100 flex-shrink-0" />
