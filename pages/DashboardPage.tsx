...
const mockProfile: Profile = {
    ...
    workExperience: [
        { id: 'we1', jobTitle: 'Frontend Developer', companyName: 'Tech Corp', startDate: '2021-01-15', endDate: '2023-08-20', description: 'Developed and maintained web applications using React and TypeScript, improving performance by 20%.' },
    ],
    education: [
        { id: 'edu1', institution: 'Universitas Teknologi', degree: 'S.Kom', fieldOfStudy: 'Ilmu Komputer', startDate: '2016-09-01', endDate: '2020-07-10' },
    ],
    skills: [
        { id: 's1', name: 'React', level: 'Advanced' },
        { id: 's2', name: 'TypeScript', level: 'Intermediate' },
        { id: 's3', name: 'Node.js', level: 'Basic' },
    ],
    applicationHistory: [
        { 
            ...
            stages: [
                ...
                { name: 'Interview HR', status: 'Dalam Proses' },
        ...
        { 
            ...
            stages: [
                { name: 'Screening', status: 'Lolos' },
                { name: 'Psikotest', status: 'Tidak Lolos' },
    ...
    documents: [
        { id: 'doc-resume-1', type: 'resume', name: 'Budi_Doremi_Resume.pdf', url: '#', uploadedAt: '2025-11-20T10:00:00Z', fileSize: 1258291 },
        { id: 'doc-cert-1', type: 'certificate', name: 'Sertifikat_React_Advanced.pdf', url: '#', uploadedAt: '2024-01-15T14:30:00Z' },
        { id: 'doc-cert-2', type: 'certificate', name: 'Completion_NodeJS_Course.png', url: '#', uploadedAt: '2023-12-20T11:00:00Z' },
    ],
};
...
const SalaryExpectation: React.FC<{ profile: Profile, setProfile: React.Dispatch<React.SetStateAction<Profile>> }> = ({ profile, setProfile }) => {
    const [isEditing, setIsEditing] = useState(false);
    const [salary, setSalary] = useState(profile.salaryExpectation);
    
    const handleSave = () => {
        setProfile(prev => ({...prev, salaryExpectation: salary }));
        setIsEditing(false);
    }
        ...
        {isEditing ? (
             <div className="space-y-4">
                <div className="flex space-x-4">
                    <div>
                        ...
                        <input type="number" value={salary.min} onChange={e => setSalary(s => ({...s, min: parseInt(e.target.value)}))} className={`mt-1 ${formInputClass}`}/>
                    </div>
                     <div>
                        <label className="text-sm font-medium text-gray-500">Maksimal (per bulan)</label>
                        <input type="number" value={salary.max} onChange={e => setSalary(s => ({...s, max: parseInt(e.target.value)}))} className={`mt-1 ${formInputClass}`}/>
                    </div>
                </div>
            ...
            <div className="flex space-x-4">
                <div>
                    ...
                    <p className="mt-1 text-gray-800 font-semibold">IDR {profile.salaryExpectation.min.toLocaleString('id-ID')}</p>
                </div>
                <div>
                    <label className="text-sm font-medium text-gray-500">Maksimal</label>
                    <p className="mt-1 text-gray-800 font-semibold">IDR {profile.salaryExpectation.max.toLocaleString('id-ID')}</p>
                </div>
            </div>
        )}
     </Card>
